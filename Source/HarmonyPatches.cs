...
namespace ConsumableGenepack
{
    ...
    public static class Building_GeneExtractor_Draw_Patch
    {
        ...
        {
            var contains = AccessTools.Method(typeof(ThingOwner), "Contains", new Type[] { typeof(Thing) });
            var containsCorpseOrPawn = AccessTools.Method(typeof(Building_GeneExtractor_Draw_Patch), nameof(ContainsCorpseOrPawn));
            return codeInstructions.MethodReplacer(contains, containsCorpseOrPawn);
        }
